...
1. The [coefficient of determination](https://scikit-learn.org/stable/modules/generated/sklearn.tree.DecisionTreeRegressor.html#sklearn.tree.DecisionTreeRegressor.score)
2. Mean error
3. Max error
3. Tree depth

I want to generate a test that is not just statistically accurate, but also likely to be accurate for each individual test-taker. This means a low mean error is critical. Furthermore, I need to know the worst case scenario (the max error). Lastly, I want to try and generate a test of a reasonable length, which correlates directly with tree depth.
